...
def default_type(_value: Any, _type: Any):
    """
    Generate a default value for the given type if the provided value is None.
    If the value is not None, attempt to cast it to the provided type.

    Handles nested types such as lists, dictionaries, and Optionals.
    """
    ...
    if get_origin(_type) is list:
        ...
        # Recursively apply default logic to each element in the list
        types = get_args(_type)
        if len(types) == 0:
            return _value

        return [default_type(item, types[0]) for item in _value]
    ...
    if get_origin(_type) is dict:
        ...
        if len(types) == 0:
            return _value
        # Recursively apply default logic to keys and values
        return {default_type(k, types[0]): default_type(v, types[1]) for k, v in _value.items()}
    ...
    if get_origin(_type) is Union or isinstance(_type, type(int | str)): # type(int | str) returns the UnionType
        ...
        for obj_type in types:
            if obj_type is Any or isinstance(_value, obj_type):
                return default_type(_value, obj_type)
    ...
    if get_origin(_type) is Literal:
        if _value in get_args(_type):
            return _value
        raise AssertionError(f"Expected {_type}, got {_value}")

    # Return the default value if value is None, otherwise cast value to the type
    return _type() if _value is None else _type(_value)
...
class TypedClass:
    ...
    def __init__(self, _data: Json):
        ...
        for attribute, annotated_type in self.__annotations__.items():
            # Use `default_type` to handle nested or complex types and set attribute value
            setattr(self, attribute, default_type(_data.get(attribute), annotated_type))
